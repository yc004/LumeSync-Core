...
const distModule = path.join(__dirname, '../../dist/cjs/node/identity.js');
...
try {
  ...
  function signHs256(content, secret) {
  ...
  function createViewerSessionToken({ clientId, ttlSec = 4 * 60 * 60, secret }) {
  ...
  function verifyViewerSessionToken(token, secret) {
    ...
    const header = safeJsonParse(fromBase64Url(headerB64).toString('utf8'));
    const payload = safeJsonParse(fromBase64Url(payloadB64).toString('utf8'));
    ...
    const nowSec = Math.floor(Date.now() / 1000);
    if (!Number.isFinite(payload.exp) || nowSec >= Number(payload.exp)) return { ok: false, code: 'token_expired' };
